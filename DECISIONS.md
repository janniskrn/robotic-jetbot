...
| D4f | 2026-09-20 | Curvature feedforward from the curve model: steering += (p_gentle x 0.015 + p_sharp x 0.04) in the direction of lane_x (only when abs(lane_x) >= 0.05). The run with D5b drove 60 s clockwise without swinging (confirmed by the user), but counterclockwise the robot ran wide out of the sharp couch curve (a left turn): P gave only about 0.036 steering at lane_x -0.25, the color-mask driver used up to 0.08 there; the right drift is small (about 0.01 on straights) | Feedforward supplies the sustained turn without adding loop delay (unlike the integral, D4b) |
